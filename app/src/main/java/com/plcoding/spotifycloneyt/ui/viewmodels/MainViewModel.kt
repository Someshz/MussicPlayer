...
class MainViewModel @ViewModelInject constructor(
    ...
    init {
        _songs.value = emptyList() // Initialize with an empty list of songs
    }

    // Function to add a single song to the existing list of songs in the ViewModel
    fun addSong(song: Song) {
        val currentList = _songs.value ?: emptyList()
        val updatedList = currentList.toMutableList()
        updatedList.add(song)
        _songs.value = updatedList
    }

    // Function to retrieve the array of songs from the ViewModel
